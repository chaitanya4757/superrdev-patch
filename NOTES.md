...
`TaskRepository.java` had a missing pair of parentheses around the `OR` in the
WHERE clause. SQL evaluates `AND` before `OR`, so the query was parsed as
`(archived=FALSE AND title LIKE term) OR (description LIKE term AND statusFilter)`.
Result: archived tasks could appear in description matches, and the status filter
was silently ignored for title matches. Fixed by wrapping the OR in parentheses.

**Bug 2 – Artificial `Thread.sleep` in TaskController**
A block labelled "Query complexity estimation for logging" sleeps up to 1000ms
per request (inversely proportional to query length — shortest queries sleep
longest). This is not estimation; it is a performance landmine. Removed entirely.

**Bug 3 – `TaskStatus.valueOf()` throws 500 on bad input**
...
**Bug 4 – `setLoading(false)` never called on error in `useTasks.js`**
...
**Bug 5 – Page not reset on filter change in `App.jsx`**
...
**Bug 6 – No debounce on search input**
...
Started by feeding the git repo link directly to Claude.ai which saved a lot of
time. After getting an initial analysis, read through the codebase to find more
bugs manually. Honestly couldn't find much more beyond what Claude flagged. There
are likely more bugs hiding in there but wanted to keep this exercise strictly
time bound. These are the bugs I was able to find and fix.
